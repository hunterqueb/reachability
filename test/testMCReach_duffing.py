        ...
        for i in prange(n_traj):
            ...
            for k in range(steps):
                if control_kind_id == _CONTROL_KIND_VELOCITY:
                    ...
                    u = u_seqs[i, k]
                x = _rk4_step_nb(x, k * dt, u, dt, omega, zeta, alpha, beta, gamma, total_mass)
                k1 = k + 1
                for s in range(n_snaps):
        ...
        for i in range(n_traj):
            ...
            for k in range(steps):
                if control_kind == "velocity":
                    ...
                    u = u_seqs[i, k]
                x = rk4_step(x, k * dt, u, dt, omega, zeta, alpha=alpha, beta=beta, gamma=gamma, total_mass=total_mass)
                k1 = k + 1
...
if __name__ == "__main__":
    ...
    snapshots, X_final = monte_carlo_reachable_set(
        ...
        steps=steps,
        n_traj=n_traj,
        control_kind="velocity",     # "uniform" or "piecewise_constant" also supported
    ...
    plot_snapshot_hulls(
        ...
        snapshots=snapshots,
        title="Duffing Oscillator Reachability (Monte Carlo Trajectories with Nominal Control)"
    )
    ...
    # propagate the nominal trajectory with full nominal control of the same type as used in MC
    ...
    for k in range(steps):
        ...
        if v > 0.0:
            ...
            u = 0.0
        x_max = rk4_step(x_max, k * dt, u, dt, omega, zeta, alpha=alpha, beta=beta, gamma=gamma, total_mass=total_mass)
        traj_max[k + 1] = x_max
    # plot max control trajectory on top of last figure
    plt.plot(traj_max[:, 0], traj_max[:, 1], 'r--', linewidth=2.5, label="nominal control trajectory")
