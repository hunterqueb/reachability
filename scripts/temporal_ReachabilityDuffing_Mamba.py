...
from qutils.ml.utils import findDecAcc, getDevice, printModelParmSize
from qutils.tictoc import timer
from qutils.ml.mamba import Mamba, MambaConfig


parser = argparse.ArgumentParser()
parser.add_argument("--model", type=str, default="mamba", help="Model name label for plots")
parser.add_argument("--horizon", type=int, default=1, help="Predict this many steps ahead (target at t+horizon)")
parser.add_argument("--lookback", type=int, default=20, help="Number of past steps fed to the model")
parser.add_argument("--hidden", type=int, default=64, help="Model hidden size (d_model)")
parser.add_argument("--layers", type=int, default=1, help="Number of Mamba layers")
parser.add_argument("--dropout", type=float, default=0.1, help="Dropout in output head")
...
parser.add_argument("--train-ratio", type=float, default=0.8, help="Ratio of trajectories to use for training")
...
parser.add_argument("--jetson", action="store_true", help="Run on jetson with smaller test size")
...
problem_dim = 2
input_size = problem_dim
output_size = problem_dim
device = getDevice()
...
def create_datasets(data_TND, lookback, horizon, train_ratio, jetson=False):
    ...
    train = data_TND[:, :split_idx, :]
    test = data_TND[:, split_idx:, :]
    ...
    def build_xy(block_TND):
        ...
        X = X.transpose(0, 2, 1, 3).reshape(W * N_, lookback, D_)
        Y = Y.reshape(W * N_, D_)
        return X, Y, W, N_
...
data_file = f"./data/test/duffing_monte_carlo_trajectories_dv_{args.dv}_dt_{args.dt}_n_{args.n}.npz"
system_data = np.load(data_file, allow_pickle=True)
dt = system_data["dt"]
trajs = system_data["trajectories"]
trajs_t = np.transpose(trajs, (1, 0, 2))
numeric_result = trajs_t
...
loader = data.DataLoader(
    data.TensorDataset(train_in, train_out),
    shuffle=True,
...
model = return_model()
model_string = args.model + ("_ood" if args.ood else "")
    ...
    with torch.no_grad():
        def eval_rmse(x_all, y_all, batch_size):
            loader_eval = data.DataLoader(
                data.TensorDataset(x_all, y_all), shuffle=False, batch_size=batch_size, pin_memory=True
            )
            se_sum = 0.0
            n_sum = 0
            preds = []
        ...
        dec_acc, err1 = findDecAcc(y_true_train, y_pred_train, printOut=False)
        dec_acc, err2 = findDecAcc(y_true_test, y_pred_test)
        _ = dec_acc
...
if args.ood:
    ood_data_file = f"./data/test/duffing_monte_carlo_trajectories_dv_{args.dv * 2}_dt_{args.dt}_n_{args.n}_ood.npz"
    ood_system_data = np.load(ood_data_file, allow_pickle=True)
    ood_trajs = ood_system_data["trajectories"]
    ood_numeric_result = np.transpose(ood_trajs, (1, 0, 2))
    _, _, test_in, test_out, _, meta = create_datasets(
        ood_numeric_result,
        lookback=args.lookback,
        horizon=args.horizon,
        train_ratio=args.train_ratio,
...
print(
    f"True Alpha-Shape Area: {area_true:.4f}, Pred Alpha-Shape Area: {area_pred:.4f}, Area Ratio (Pred/True): {area_ratio:.4f}"
)
...
ax.autoscale_view()
plt.title(f"{model_string} Final-State Alpha Shapes: Area Ratio {area_ratio:.4f}")
plt.xlabel("x1")
plt.ylabel("x2")
plt.legend(loc="best")
plt.savefig(f"plots/{model_string}_final_state_alpha_shapes_ratio_{args.train_ratio}_epoch_{args.n_epochs}_lr_{args.lr}.png")
plt.close()

plt.figure()
plt.scatter(final_true[:, 0], final_true[:, 1], s=6, alpha=0.4, label="True Final States")
plt.scatter(final_pred[:, 0], final_pred[:, 1], s=6, alpha=0.4, label="Pred Final States")
plt.title(f"{model_string} Final-State Points")
plt.xlabel("x1")
plt.ylabel("x2")
plt.legend(loc="best")
plt.savefig(f"plots/{model_string}_final_state_points_ratio_{args.train_ratio}_epoch_{args.n_epochs}_lr_{args.lr}.png")
plt.close()
